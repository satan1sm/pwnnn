...
但有个坑爹的地方，注意 ropme() 里的函数位址，全都是 0x08**0a**xxxx！！中间包含了 `0xa`，如果我们的 payload 包含 0xa ，从该处开始就会被 gets() 截断，所以我们没办法 return 到 ropme() 里面的任何一处

<br>

那么只能按照题目意思乖乖来了，执行 A(), B(), ... , G() 并计算 EXP 的总和，最后一次 ret 返回到 main() 里面重新呼叫一次 ropme()，然后输入正确的 exp 总和，flag 就会打印出来了。
